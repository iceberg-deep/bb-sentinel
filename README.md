...
```bash
git clone https://github.com/iceberg-deep/bb-sentinel.git
cd bb-sentinel
...
programs:
  evilcorp:
    ...
    # Per-program outbound rate cap (matches program-published limits)
    rate_limit_rps: 5
...
and (c) keeps the published code a generic SSRF tool while
...
| `BBSENTINEL_PIVOT_URLS` | Comma-separated list of internal targets. When set **alongside** `OOB_HOST`, `ssrf-oob` enters **internal-pivot** mode — after the OOB payload, also injects each pivot URL and flags critical when the canary appears in the response. | `http://10.72.218.66/flag.txt,http://10.62.252.214/flag.txt` |
...
program explicitly authorizes. T-Mobile-style briefs that list
...
— programs that publish their own `submission-template.md` in the
