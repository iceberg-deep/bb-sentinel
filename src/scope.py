...
class InscopeFilter:
    ...
    When the binary is missing or no scope file is configured, the filter is a pass-through.
    """
    ...
    async def filter(self, items: list[str]) -> tuple[list[str], set[str]]:
        ...
        cmd = [self.binary_path, self.scope_file]  # type: ignore[list-item]
        ...
        if proc.returncode != 0:
            log.error("inscope failed — failing open", rc=proc.returncode, stderr=stderr.decode(errors="replace")[:200])
            return list(items), set(items)
