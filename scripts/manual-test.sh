...
#   discover <seed-host>           TLS-SAN transitive (no T-Mobile HTTP needed)
...
#   BB_PROGRAM       default program (default: tmobile-internal-zones)
#   BB_RATE          rate-limit r/s (default: 3)
#   BB_BC_USERNAME   Bugcrowd username for X-Bug-Bounty (default: iceberg-deep)
...
#
# Examples:
#   ./scripts/manual-test.sh discover passwordreset.internal.t-mobile.com
#   ./scripts/manual-test.sh probe papi.t-mobile.com
#   ./scripts/manual-test.sh probe-list /tmp/some-targets.txt
#   ./scripts/manual-test.sh scan tmobile-internal-zones
#   BB_PROGRAM=tmobile-core ./scripts/manual-test.sh rocks
#

set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PROGRAM="${BB_PROGRAM:-tmobile-internal-zones}"
RATE="${BB_RATE:-3}"
BC_USER="${BB_BC_USERNAME:-iceberg-deep}"
